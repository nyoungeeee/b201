...
const getReservationOccurrenceEndAt = (reservation: ReservationListItem) => {
    const startAt = createDateTimeString(reservation.start_date, reservation.start_time);
    const endAt = createDateTimeString(reservation.start_date, reservation.end_time);

    if (new Date(endAt) > new Date(startAt)) return endAt;

    return createDateTimeString(addDays(reservation.start_date, 1), reservation.end_time);
};
...
export const mapReservationListItem = (
    ...
    const isRepeat = options.isRepeat ?? reservation.kind === 'repeat';
    ...
    return {
        ...
        endAt: isRepeat
            ? getReservationOccurrenceEndAt(reservation)
            : getReservationEndAt(reservation),
